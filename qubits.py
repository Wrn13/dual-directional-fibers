...
def get_Hamiltonian(c):
    return (pauli_prods[None, :, :, :] * c[:, :, None, None]).sum(
        dim=1
    )  # (K, 2**n_qb, 2**n_qb)
...
if __name__ == "__main__":
    ...
    if do_fiber:
        ...
        # c_targ = tr.randn(1,9)
        # TODO: Determine if Hamiltonian spectrum is degenerate
        # Ordering: a_03, a_11, a_13, a_21, a_22, a_23, a_31, a_32, a_33
        # Isz, sxsy, sxsz, sysx, sysy, sysz, szsx, szsy, szsz
        c_targ = single_dual_params.c_vector
        ...
        c_dir = single_dual_params.c_direction.T
    ...
    pt.title("Loss Landscape on XYZ Hamiltonian")
