...
def get_Hamiltonian(c):
    """Creates the reduced Hamiltonians with boundary terms"""
    ...
    for i in range(n_qb - 1):
        ...
        H += c[:,0].view(K, 1, 1) * kron_prod(interaction_list) + c[:, 1].view(K, 1, 1) * kron_prod(single_term)
    ...
    H += c[None, :, 2].view(K, 1, 1) * kron_prod(site_one_x) + c[None, :, 3].view(K, 1, 1) * kron_prod(site_N_x) + c[None, :, 4].view(K, 1, 1) * kron_prod(site_N_z)
...
def run_fiber(starting_point, direction, f, Df, get_loss, output_file):
    fiber_kwargs = {
        ...
        "terminate": lambda trace: (
            get_loss(tr.tensor(trace.x[:5].T)) > 1
        ).any(),
        "max_step_size": 100,
...
def main():
    ...
    # constant direction
    c_dir = np.array([[rng.normal() for _ in range(5)]]).T
    h = 1.5
    J = 1
    #ZZ_i X_i X_1 X_n Z_n
    c_targ = tr.tensor([[J, h, 0, h, 0]])
    ...
    if do_fiber:
        run_fiber(v0, c_dir, f, Df, get_loss, "results/ising_seed_1.pkl")

    with open("results/ising_seed_1.pkl", "rb") as file:
    ...
    #Peaks at 1, 38, 123
    normal_dir = np.array([1,0,0,0,0])
    # Restart at around 38
    if do_perturb1:
        init_vector = V[:, 38] + 0.5* normal_dir
        init_vector = np.atleast_2d(init_vector).T
        run_fiber(init_vector, c_dir, f, Df, get_loss, "results/ising_seed_1_stitch_1.pkl")

    with open("results/ising_seed_1_stitch_1.pkl", "rb") as file:
    ...
    if do_perturb2:
        init_vector = V[:, 123] + 0.5 * normal_dir
        init_vector = np.atleast_2d(init_vector).T
